...
class CourseController extends PublicController {
    ...
    public function index(){
        ...
        $detail['photo_xq'] = __DATAURL__.$detail['photo_xq'];
    ...
    public function getlist(){
        $list = M('course')->where('del=0 AND is_show=1')->field('id,title,yu_price')->select();
        foreach ($list as $k => $v) {
            if (floatval($v['yu_price'])>0) {
                $list[$k]['title'] = $v['title'].'(预付:'.floatval($v['yu_price']).'元)';
            }else{
