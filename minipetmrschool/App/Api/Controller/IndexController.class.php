...
class IndexController extends PublicController {
    ...
    public function index(){
        ...
        //首页培训课程
        //======================
        $course = M('course')->where('del=0 AND is_show=1')->order('id desc')->field('id,title,intro,photo')->limit(6)->select();
        ...
        //首页分类 自己组建数组
        //======================
        $indeximg = M('indeximg')->where('1=1')->order('sort asc')->select();
        foreach ($indeximg as $k => $v) {
            $indeximg[$k]['imgs'] = __DATAURL__.$v['photo'];
        }

        //优惠券信息
        $vouinfo = M('indexmodel')->where('id=1')->find();
        if ($vouinfo['photo']) {
            $vouinfo['photo'] = __DATAURL__.$vouinfo['photo'];
        }

    	echo json_encode(array('ggtop'=>$ggtop,'procat'=>$indeximg,'prolist'=>$pro_list,'brand'=>$brand,'course'=>$course,'vouinfo'=>$vouinfo));
    	exit();
    }
