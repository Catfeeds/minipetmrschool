...
class WxpayController extends Controller{
	...
	public function wxpay(){
		...
		$input->SetBody("宠物美容学校商品购买_".trim($order_info['order_sn']));
		$input->SetAttach("宠物美容学校商品购买_".trim($order_info['order_sn']));
		$input->SetOut_trade_no($pay_sn);
		$input->SetTotal_fee(floatval($order_info['amount'])*100);
		$input->SetTime_start(date("YmdHis"));
		$input->SetTime_expire(date("YmdHis", time() + 3600));
		$input->SetGoods_tag("宠物美容学校商品购买_".trim($order_info['order_sn']));
		$input->SetNotify_url('https://gzleren.com/minipetmrschool/index.php/Api/Wxpay/notify');
	...
	public function buycourse(){
		...
		$data['order_type'] = 2;
		//$oprice = floatval($check_course['yu_price']);
		$oprice = floatval(0.01);
	...
	public function orderhandle($data){
		...
		if ($res) {
			...
			if ($ptype==2) {
				//课程订单
				$course_id = M('order_product')->where('order_id='.intval($check_info['id']))->getField('pid');
