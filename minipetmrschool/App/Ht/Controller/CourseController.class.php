...
class CourseController extends PublicController{
	...
	*/
	public function save(){
		...
		if (!empty($_FILES["files"]["tmp_name"])) {
			//文件上传
			$info = $this->upload_images($_FILES["files"],array('jpg','png','jpeg'),"course/".date(Ymd));
			if(!is_array($info)) {// 上传错误提示错误信息
				$this->error($info);
				exit();
			}else{// 上传成功 获取上传文件信息
				$array['photo_xq'] = 'UploadFiles/'.$info['savepath'].$info['savename'];
				$xt = M('course')->where('id='.intval($id))->field('photo_xq')->find();
				if (intval($id) && $xt['photo_xq']) {
					$img_url = "Data/".$xt['photo_xq'];
					if(file_exists($img_url)) {
						@unlink($img_url);
					}
				}
	...
	*/
	public function upshow () {
		...
		$is_show = intval($check_info['is_show'])?0:1;

		$res = M('course')->where('id='.intval($id))->save(array('is_show'=>$is_show));
		if ($res) {
			$this->success('操作成功.');
			exit();
		}else{
	...
	*/
	public function del(){
		...
		if ($res) {
			...
			$xq = $check_info['photo_xq'];
			if ($xq) {
				$img_urls = "Data/".$xq;
				if(file_exists($img_urls)) {
					@unlink($img_urls);
				}
			}
			$this->redirect('index');
