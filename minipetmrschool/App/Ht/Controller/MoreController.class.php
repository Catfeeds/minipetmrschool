...
class MoreController extends PublicController{
	...
	public function setvou(){
		if(IS_POST){
			...
			if (!empty($_FILES["file"]["tmp_name"])) {
				//文件上传
				$info2 = $this->upload_images($_FILES["file"],array('jpg','png','jpeg'),"public");
			    if(!is_array($info2)) {// 上传错误提示错误信息
			        $this->error($info2);
			        exit();
			    }else{// 上传成功 获取上传文件信息
				    $data['photo'] = 'UploadFiles/'.$info2['savepath'].$info2['savename'];
			    }
			}

			if ($data) {
				$up = M('indexmodel')->where('id='.intval($_REQUEST['id']))->save($data);
				if ($up) {
					$this->success('保存成功！');
					exit();
				}else {
				...
				$this->error('没有找到可修改的数据！');
				exit();
			}
			
		}else{
			$this->assign('info',M('indexmodel')->where('id='.intval($_REQUEST['id']))->find());
			$this->display();
		}
